// ESLint有三种配置方式，一般自定义规则较少时，选第2种；自定义配置很多，建议第1种；第3种常用于对某句代码禁用eslint。
/* 1、在package.json中通过"eslintConfig":{} 对象进行配置
例:"eslintConfig": {
        "root": true,
        "env": {
          "node": true
...
*/
// 2、通过项目根目录的.eslintrc.js文件配置，支持JSON和YAML两种语法
// 3、通过注释在代码中配置，注释位置有讲究
module.exports = {
	root: true,
	env: {
	...
	// 配置文件可以被基础配置中的已启用的规则继承 值为字符串或者字符串数组：每个配置继承它前面的配置
	extends: [
		'plugin:vue/essential', // 插件配置? 反正VUE就必须加这个
		'eslint:recommended', // 启用默认规则配置
		'@vue/standard' // 插件配置? 反正VUE就必须加这个
	],
	// 解析器选项
	parserOptions: {
		parser: 'babel-eslint'
	},
	// 规则配置选项: 还可以根据环境变量来区分不同的环境 配置不同的规则 例: no-console: process.env.NODE_ENV === "production" ? "warn" : "off"
	rules: {
		indent: 0, // 强制一致的缩进风格
		'no-tabs': 0, // 禁用tab
